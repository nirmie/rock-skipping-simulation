...
export function setupUI({
  ...
  if (water.simulationMaterial) {
    ...
    simFolder.addBinding(
      water.simulationMaterial.uniforms.uViscosity,
      "value",
      {
        min: 0,
        max: 0.3,
    ...
    simFolder.addBinding(
      water.simulationMaterial.uniforms.uDisturbanceAmount,
      "value",
      {
        min: 0.5,
        max: 2,
        step: 0.1,
        label: "disturbance amount",
      }
    );
    // Bind disturbance radius control
    simFolder.addBinding(
      water.simulationMaterial.uniforms.uDisturbanceRadius,
      "value",
      {
        min: 0.001,
        max: 0.05,
        step: 0.001,
        label: "disturbance radius",
      }
    );
  ...
  if (water.causticsMaterial) {
    ...
    causticsFolder.addBinding(
      water.causticsMaterial.uniforms.uIntensity,
      "value",
      {
        min: 0,
        max: 5,
  ...
  if (ground.material) {
    ...
    groundFolder.addBinding(
      ground.material.uniforms.uTextureRepeat.value,
      "x",
      {
        min: 1,
        max: 20,
  ...
  const exGroundFolder = pane.addFolder({
    title: "Exterior Ground",
    expanded: false,
  });
  if (ground.exteriorMaterial) {
    // Bind to the intermediate string state
    exGroundFolder
      .addBinding(uiState, "exGroundTexture", {
        label: "Texture",
        options: {
          Riverbed: "Riverbed",
          Sand: "Sand",
    ...
    exGroundFolder.addBinding(
      ground.exteriorMaterial.uniforms.uTextureRepeat.value,
      "x",
      {
        min: 1,
        max: 20,
  ...
  if (rockThrowController) {
    const rockAppearanceFolder = pane.addFolder({
      title: "Rock Appearance",
      expanded: false,
    });

    // Rock type selection
    const rockTypeConfig = {
      type: rockThrowController.rockOptions.rockType, // Initialize with current value
    };

    // Create options object for dropdown
    const rockTypeOptions = {};
    Object.keys(rockTypes).forEach((type) => {
    ...
    rockAppearanceFolder
      ...
      .on("change", ({ value }) => {
        ...
        console.log(
          `Rock type changed to ${value} (applies to newly thrown rocks)`
        );
      });

    // Displacement scale control
    rockAppearanceFolder
      .addBinding(rockThrowController.rockOptions, "displacementScale", {
        min: 0,
        max: 0.2,
        step: 0.01,
    ...
    rockAppearanceFolder
      .addBinding(rockThrowController.rockOptions.textureRepeat, "x", {
        min: 1,
        max: 10,
        step: 0.5,
    ...
    rockAppearanceFolder
      .addBinding(rockThrowController.rockOptions.textureRepeat, "y", {
        min: 1,
        max: 10,
        step: 0.5,
    ...
    const rockFolder = pane.addFolder({
      title: "Rock Physics",
      expanded: false,
    });
    ...
    rockFolder
      .addBinding(rockThrowController.rockOptions, "throwVelocity", {
        min: 1.0,
        max: 15.0,
        step: 0.5,
    ...
    rockFolder
      .addBinding(rockThrowController.rockOptions, "minSkipVelocity", {
        min: 0.2,
        max: 2.0,
        step: 0.1,
        label: "Min Skip Velocity",
      })
      .on("change", () => {
        rockThrowController.updateAllRocksOptions();
      });
    ...
    rockFolder
      .addBinding(rockThrowController.rockOptions, "skipsBeforeSink", {
        min: 1,
        max: 10,
        step: 1,
        label: "Max Skips",
