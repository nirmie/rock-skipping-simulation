...
void main() {
    ...
    float fresnel = pow(1.0 - clamp(dot(viewDirection, vNormal), 0.0, 1.0), uFresnelPower) * uFresnelStrength;

    float trough2surface = smoothstep(uTroughThreshold-uTroughTransition, uTroughThreshold + uTroughTransition, vWorldPosition.y);
    float surface2crest = smoothstep(uCrestThreshold-uCrestTransition, uCrestThreshold + uCrestTransition, vWorldPosition.y);
